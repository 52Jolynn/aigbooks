...
3. 可选：**封面图**（默认占位图，单文件最大 20MB）、**证据**（图片/视频，可多份；每个证据文件最大 50MB，不设置业务总量上限；文字证据即描述字段）
4. 表单提供三种本地识别模式：上传图片、拍照识别、实时条码扫描。上传和拍照按“条码识别 → PaddleOCR 文字识别”顺序执行；每一步得到结果后立即回填表单，条码失败不阻断 OCR，编号冲突时条码优先。OCR 文本按行解析：第一行作为题名，著者/作者标签优先提取作者，ISBN/书号行提取编号，其余行仅在摘要为空时回填；ISBN-13 使用 `978-7-5086-7553-4` 格式展示。实时条码扫描只回填 ISBN/ISSN，不追加 OCR
...
| 封面超限（单文件 20MB） | 后端校验返回 413 |
| 证据超限（单文件 50MB） | 后端逐文件校验返回 413；不限制多文件业务总量，代理层保留请求级安全上限 |
